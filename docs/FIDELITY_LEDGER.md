...
| --- | --- | --- |
| A judge landed directly inside a dense control plane with no explanation of provenance or playback limits | `/` is now a compact judge briefing; the existing control plane moved to `/replay`. The first screen states that the current process is deterministic, performs no model calls, and replays the committed 89-event record. It links the exact threads, tokens, diff digest, live-evidence document, and one-command local path without presenting playback as a new invocation. | Closed and verified in the deployed production browser. The landing `fixture` badge labels the playback process while replay cards preserve historical event provenance. |
| Replay was a dense dashboard table with no cinematic causal reveal | Consequential records project into 38 vertical evidence cards controlled by one scrubber-owned GSAP timeline. Cards reveal with transform/opacity, SVG links draw by stroke offset, and predecessor hashes pulse in order. | Closed; the first run card identifies that its predecessor is outside the run view, while the remaining visible links make the projected chain legible. |
...
| Economy panel exposed only a transaction list and a partial roster | The ledger shows all eight agents, state, balance bar, and cursor-time balance. Settlement plays as winner/Aegis/Sentinel/Shipwright/Chronicle/refund transfers and proves the sum beside escrow. Penalties use restrained red and a 2 px shake; dormant agents grayscale. | Closed; final-run settlement is `24 + 5 + 5 + 3 + 2 + 1 = 40`. |
| Recovery was buried among general replay records | Regression, alert, four recorded liability penalties, known-good restore, policy proposal, and the human decision form a guided recovery rail. | Closed; the final captured proposal remains `proposed`, so the UI truthfully ends at an awaiting-human-decision gate. |
...
- Production Vite preview and the deployed Render landing/replay routes were inspected in the in-app browser against the final 89-event journal.
