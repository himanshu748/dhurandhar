...
describe("Dhurandhar replay", () => {
  ...
  it("renders the captured self-hosting run and reviewer evidence", async () => {
    render(<App />);
    expect(await screen.findByText("Build the Change Replay interface")).toBeInTheDocument();
  ...
  it("selects a different event from the causal timeline", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Monitor detected regression" })).toBeInTheDocument();
  });
  ...
  it("stays visibly read-only until an in-memory operator token is loaded", async () => {
    ...
    expect(await screen.findByRole("button", { name: /operator token loaded/i })).toBeInTheDocument();
    ...
    expect(await screen.findByRole("button", { name: /read-only/i })).toBeInTheDocument();
    expect(createButton).toBeDisabled();
  });
  ...
  it("opens the objective creation dialog after operator access is loaded", async () => {
    ...
    expect(await screen.findByLabelText("Objective")).toBeInTheDocument();
  });
  ...
  it("keeps the cursor and evidence inspector together during keyboard seek", async () => {
    ...
    expect(await screen.findByRole("slider", { name: "Replay position" })).toHaveValue("0");
    expect(await screen.findByRole("heading", { name: "Brief accepted" })).toBeInTheDocument();
  });
  ...
  it("shows the complete eight-agent company with capabilities and learned memory", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Eight-agent company" })).toBeInTheDocument();
    ...
    expect(await screen.findByText("Keep replay state derived from one sequence cursor.")).toBeInTheDocument();
  });
  ...
  it("replays competitive bids and the winning engineer", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Implementation auction awarded" })).toBeInTheDocument();
    expect(await screen.findByRole("heading", { name: "Task auction" })).toBeInTheDocument();
    expect(await screen.findByText("Winning engineer")).toBeInTheDocument();
    expect((await screen.findAllByText("Prism · Frontend Engineer")).length).toBeGreaterThan(0);
    expect(await screen.findByText(/3 bids considered/)).toBeInTheDocument();
  });

  it("labels the offline replay as deterministic fallback", async () => {
    render(<App />);
    await screen.findByText("Build the Change Replay interface");
    expect((await screen.findAllByText("Deterministic fallback")).length).toBeGreaterThan(0);
    expect(await screen.findByText(/No model call produced this event/)).toBeInTheDocument();
  });
});
